...
mongoose.connect(process.env.MONGO_URI)
  .then(async () => {
    console.log("✅ MongoDB Connected: HeavyApp POS Database");
...
// Health check endpoints to prevent 404 errors during connection pings
...
app.post('/api/shops/request-link', async (req, res) => {
  ...
  try {
    ...
    if (!shop) {
      console.log(`❌ Shop lookup failed for code: '${branchCode}'. Ensure prefix is correct (e.g., BATT-).`);
