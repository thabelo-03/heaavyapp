  ...
  useFocusEffect(
    ...
    const fetchData = async () => {
      try {
        ...
        if (!userResponse.ok) {
          // If user fetch fails (e.g. deleted user, invalid token), logout
        ...
        if (e instanceof Error) {
            // Instead of showing an error, attempt to log out
            await AsyncStorage.clear();
            router.replace('/(auth)/login');
        } else {
  ...
  useEffect(() => {
    ...
    if (diffDays <= 14) {
      for (let i = 0; i < diffDays; i++) {
        ...
        labels.push(d.toLocaleDateString('en-US', { day: 'numeric' }));
